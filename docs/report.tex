...
\section{Method}
The pipeline has six stages. The main program limits each image's largest dimension to 1280 pixels before processing; detections are then mapped back to the original scene coordinates for display.
...
After accepting a model, the template corners are projected through it and converted to an axis-aligned bounding box. Its RANSAC inliers are removed from the active match list, then Hough voting and verification repeat to seek another instance. Finally, detections are ranked by their inlier counts and standard IoU non-maximum suppression removes lower-scoring boxes whose overlap is at least 0.5.
...
On the supplied images, the implementation extracted 1138 template keypoints and 2000 scene keypoints. Lowe's ratio test retained 279 matches. The greedy stage accepted 25 affine hypotheses and accumulated 176 verified inlier matches; IoU NMS reduced these to four final candidate boxes. These are pipeline measurements, not ground-truth accuracy metrics: the scene has not been manually annotated, so precision and recall are deliberately not claimed.
...
\begin{figure}[htbp]
    ...
    \caption{Naive ratio-test matches. The dense set of lines illustrates that descriptor similarity alone does not provide a clean instance-level decision.}
    \label{fig:naive}
\end{figure}

Figure~\ref{fig:verified} contains only matches retained as RANSAC inliers over the greedy extraction process. Relative to Figure~\ref{fig:naive}, the evidence is more structured, but correspondence overlays still do not communicate the final localization as clearly as boxes.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{verified_matches.jpg}
    \caption{Geometrically verified matches retained by the affine-RANSAC stage.}
    \label{fig:verified}
\end{figure}

The final output in Figure~\ref{fig:final} is much easier to read than the raw match overlay. It places several boxes on notebook instances across different rotations and scales. However, it also retains candidate boxes that do not cleanly correspond to a notebook. The headset and large regions of background are particularly challenging. Therefore, the correct interpretation is not ``perfect detection'' but a substantial reduction of noisy matches into a small, inspectable set of hypotheses.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\linewidth]{final_detections.jpg}
    \caption{Final boxes after affine verification, greedy extraction, and IoU non-maximum suppression. Green boxes are candidate detections scored by RANSAC inlier count. Some false positives remain in this highly cluttered scene.}
    \label{fig:final}
\end{figure}
